...
public class UserDAOImpl implements IDAO<User, String> {
	...
	public void delete(String target) {
		...
		try {
			PreparedStatement stmt = connection.getConnection().prepareStatement("DELETE FROM " + name + " WHERE username=?");
	...
	public void add(User addition) {
		...
		try {
			PreparedStatement stmt = connection.getConnection().prepareStatement("INSERT INTO "
																				 + name
																				 + " (username, password, permission) VALUES (? , ? , ? )");
			stmt.setString(1, addition.getUserName());
			stmt.setString(2, addition.getPassword());
			stmt.setInt(3, addition.getPermission().ordinal());
			stmt.execute();
		} catch (SQLException e) {
			throw new RuntimeException(e);
		}
	...
	public void add(String username, String password, User.Permission permission) {
		User addition = new User(username, password, permission);
		this.add(addition);
	}
