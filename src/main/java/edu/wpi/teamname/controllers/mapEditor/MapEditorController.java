...
public class MapEditorController {
  ...
  private List<Circle> undoAlignment = new ArrayList<>();
  ...
  private void initCircle(@NotNull Circle circle) {
    ...
    circle.setOnMouseClicked(
        event -> {
          if (mode == MoveState.MAKE_ALIGNMENT_LINE && alignmentLine != null) {}
        });
  ...
  private void deleteCircle(Circle circle) {
    ...
    if (queueManager.queue.contains(node)) queueManager.queue.remove(node);
  ...
  private void deleteLine(Line line) {
    ...
    if (queueManager.queue.contains(edge)) queueManager.queue.remove(edge);
    lines.remove(line);
    ...
    mapPane.setOnKeyPressed(
        event -> {
          if (!(mode == MoveState.ALIGN || mode == MoveState.MAKE_ALIGNMENT_LINE)
              || ((mode == MoveState.ALIGN || mode == MoveState.MAKE_ALIGNMENT_LINE)
